...
test("README.md", () => {
  ...
  expect(JSON.stringify(flatDeep(arr))).toBe(
    JSON.stringify([0, "hoge", 1, 2, 3, 4, 5, 6])
  );
});
...
test("flat any[]", () => {
  const map = new Map();
  const arr = ["hoge", [[map, 2], "string", [0, 2, 3, 4, 5]]];
